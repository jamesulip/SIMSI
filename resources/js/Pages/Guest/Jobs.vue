...
const computedRecentJobs = computed(() => {
  return recentJobs.data.map((job: any) => {
    return {
      title: job.title,
      href: `/job/${job.uuid}`,
      category: {
...
<template>
  <GuestVue>
    <div class="w-full flex h-96 relative bg-cover bg-bottom bg-gradient-to-r from-cyan-500 to-blue-500"
      ...
      <div class="absolute top-1/2 left-1/2 transform text-center -translate-x-1/2 -translate-y-1/2">
          ...
          <div class="sm:grid grid-cols-4 gap-2 mx-auto max-w-5xl">
            <div class="flex-0 mb-3">
              <q-form @submit="searchJob()" @reset="resetSearch" class="flex gap-2">
                <q-input class="w-full" v-model="search.search" outlined dense label="Job Title" name="search"
                  clearable>
                </q-input>
                <q-select use-input dense :options="available_locations" @filter="filterFn" v-model="search.location"
                  outlined label="Location" :loading="isFetching" :option-label="(v) => v?.location" map-options
                  emit-value class="w-full" :option-value="(b) => b?.location" />
                <div class="flex w-full gap-3">
                  <q-btn icon="close" flat class="w-full" type="reset"></q-btn>
                  <q-btn class="w-full flex-1" icon="search" type="submit" color="green" label="Search"></q-btn>
                </div>
              </q-form>
            </div>
            <div class="col-span-3 gap-3 grid">
              ...
              <div v-if="computedRecentJobs.length == 0"
                class="flex flex-col border overflow-hidden rounded-lg  border-gray-200">
              ...
              <div class="flex flex-1 justify-between sm:justify-end">
                <a :href="jobs.prev_page_url" v-if="jobs.prev_page_url"
                  class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Previous</a>
                <a :href="jobs.next_page_url" v-if="jobs.next_page_url"
                  class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Next</a>
              </div>
            </div>

            <!-- <q-table :rows="computedRecentJobs" :columns="cols" class="w-full flex-1 col-span-3 ">
              <template #body-cell-imageUrl="{ row }">
                <q-td>
                  <q-img :src="row.imageUrl" alt="" />
                </q-td>
              </template>
              <template #body-cell-title="{ row }">
                <q-td>
                  <div class="text-caption font-semibold">{{ row.title }}</div>
                  <div class="max-w-xs lineclamp">
                    <span v-html="row.description" class="text-caption ">
